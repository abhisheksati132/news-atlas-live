export default async function handler(req, res) {
  ...
  try {
    ...
    if (type === "crypto") {
      ...
      return res
        .status(200)
        .json({ type: "crypto", currency: cur, data: result });
    } else if (type === "forex") {
      ...
      majorPairs.forEach((p) => {
        if (data.rates[p]) rates[p] = data.rates[p];
      });
      ...
      const results = await Promise.all(
        metalsList.map(async (m) => {
          try {
            ...
            const prev =
              meta.chartPreviousClose || meta.previousClose || priceUSD;
            const change = prev
              ? +(((priceUSD - prev) / prev) * 100).toFixed(2)
              : 0;
      ...
      return res
        .status(200)
        .json({ type: "commodities", currency: cur, data: commodityData });
    }
    return res
      .status(400)
      .json({ error: "Invalid type. Use: crypto, forex, metals, commodities" });
  } catch (err) {
    return res
      .status(500)
      .json({ error: "Markets fetch failed", detail: err.message });
  }
}
