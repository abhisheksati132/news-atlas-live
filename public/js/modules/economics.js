...
async function drawGDPTrend(country) {
    ...
    try {
        ...
        renderGDPCanvas(ctx, canvas, raw.map(d => d.value / 1e9), raw.map(d => d.date));
    } catch (_) {
...
// ─── ECB: LIVE EUROPEAN CENTRAL BANK RATES (no key) ──────────────────────────
async function fetchECBRates() {
    ...
    try {
        // ECB SDMX free API — EUR reference rates
        const pairs = [
        ...
        const results = await Promise.allSettled(pairs.map(p =>
            fetch(`https://data-api.ecb.europa.eu/service/data/EXR/${p.key}?format=jsondata&lastNObservations=2`)
                .then(r => r.json())
        ));
        container.innerHTML = '';
        results.forEach((r, i) => {
