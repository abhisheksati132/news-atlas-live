...
window.upgradeToGoogle = async () => {
  ...
  if (auth.currentUser && !auth.currentUser.isAnonymous) {
    const confirmLogout = confirm(
      "⚠️ COMMANDER: Do you want to terminate this session?",
    );
    if (confirmLogout) {
  ...
  try {
    ...
    if (window.showToast)
      window.showToast(
        `WELCOME COMMANDER ${user.displayName.split(" ")[0].toUpperCase()}`,
        "success",
      );
  } catch (error) {
    ...
    if (error.code === "auth/popup-blocked") {
      alert(
        "Security Warning: Popup Blocked. Please allow popups for this site.",
      );
      return;
    }
