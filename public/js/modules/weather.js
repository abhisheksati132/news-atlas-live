...
function getMoonPhase() {
    ...
    if (b >= 8) b = 0;
    const phases = [
        { t: "New Moon", i: "fa-circle" }, { t: "Waxing Crescent", i: "fa-moon" },
        { t: "First Quarter", i: "fa-adjust" }, { t: "Waxing Gibbous", i: "fa-moon" },
        { t: "Full Moon", i: "fa-circle text-white" }, { t: "Waning Gibbous", i: "fa-moon" },
        { t: "Last Quarter", i: "fa-adjust" }, { t: "Waning Crescent", i: "fa-moon" }
    ];
    return phases[b];
}
async function fetchWeather(lat, lon) {
    ...
    try {
        ...
        if (data.current) {
            ...
            if (document.getElementById('atmo-feels')) document.getElementById('atmo-feels').innerText = `${Math.round(curr.apparent_temperature)}°`;
            if (document.getElementById('atmo-wind-speed')) document.getElementById('atmo-wind-speed').innerText = Math.round(curr.wind_speed_10m);
            ...
            if (hourlyContainer) {
                ...
                try {
                    for (let i = currentHour; i < currentHour + 24; i++) {
                        ...
                        const hMeta = getWeatherMeta(hCode, (i % 24) > 6 && (i % 24) < 18 ? 1 : 0);
        ...
        try {
            ...
            generateWeatherAnalysis(weatherSummary, window.selectedCountry ? window.selectedCountry.properties.name : 'this location');
        } catch (err) { }
...
async function generateWeatherAnalysis(weatherData, cityName) {
  ...
  In 150 words cover: Overall assessment, travel advisories, health warnings (UV/air), outdoor impact, recommended actions.`;
        ...
        const data = await res.json();
        el.innerText = data.candidates?.[0]?.content?.parts?.[0]?.text || 'Weather analysis unavailable.';
...
function generateWeatherAlerts(weatherData) {
    ...
    if (weatherData.uv_index >= 8) alerts.push({ type: 'warning', icon: '☀️', title: 'High UV Index', description: `UV Index ${weatherData.uv_index}. Wear sunscreen and protective clothing.` });
