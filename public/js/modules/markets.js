...
async function displayPreciousMetals() {
    ...
    try {
        ...
        Object.entries(json.data || {}).forEach(([sym, data]) => {
            ...
            card.className = 'bg-black/20 rounded-xl p-3 border border-white/5 relative overflow-hidden group';
            card.innerHTML = `
...
async function displayForex() {
    ...
    try {
        ...
        Object.entries(json.rates || {}).filter(([c]) => c !== base).slice(0, 16).forEach(([pair, rate]) => {
            const card = document.createElement('div');
            card.className = 'bg-black/20 rounded-xl p-3 border border-white/5 flex justify-between items-center';
            card.innerHTML = `
                <div>
