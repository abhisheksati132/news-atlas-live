...
async function fetchNews(overrideQ) {
    ...
    try {
        ...
        let url;
        if (q && q.trim()) {
            // When searching, include iso2 to blend country feeds + search filter
...
// ─── GDELT: GLOBAL EVENT INTELLIGENCE (no key) ───────────────────────────────
async function fetchGDELTEvents(country) {
    const container = document.getElementById('gdelt-events-content');
    if (!container) return;
    container.innerHTML = '<div class="text-slate-500 text-xs animate-pulse py-2">Scanning GDELT intelligence matrix...</div>';
    try {
        // GDELT Doc API v2 — theme-based event search for the selected country
        ...
        if (!articles.length) {
            container.innerHTML = '<div class="text-slate-500 text-xs py-2">No GDELT events in last 24h for this region.</div>';
            return;
        }
        container.innerHTML = '';
        articles.slice(0, 8).forEach(a => {
            ...
            row.className = 'py-2 border-b border-white/5 cursor-pointer hover:bg-white/2 transition-colors';
            row.innerHTML = `
