...
export default async function handler(req, res) {
    ...
    try {
        ...
        if (data.errors) {
            return res.status(502).json({ status: "error", message: data.errors[0] || "GNews error" });
        }
        ...
        const payload = { status: "success", totalResults: data.totalArticles || results.length, results };
