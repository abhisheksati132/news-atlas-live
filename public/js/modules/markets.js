...
async function displayPreciousMetals() {
  ...
  if (!container) return;
  container.innerHTML =
    '<div class="col-span-3 text-slate-500 text-xs py-2">Fetching live prices...</div>';
  try {
    ...
    Object.entries(json.data || {}).forEach(([sym, data]) => {
      ...
      const changeClass =
        (data.change || 0) >= 0 ? "text-emerald-400" : "text-red-400";
    ...
    if (container.children.length === 0)
      container.innerHTML =
        '<div class="text-slate-500 text-xs col-span-3">No metals data</div>';
  } catch (e) {
    container.innerHTML =
      '<div class="text-slate-500 text-xs col-span-3">Data unavailable</div>';
  }
}
async function displayCountryIndices(countryName) {
  ...
  try {
    ...
    const json = await res.json();
    container.innerHTML = "";
    const indices = (json.data || []).filter(
      (d) =>
        !["BTC-USD", "ETH-USD", "GOLD", "CRUDE OIL", "EUR/USD"].includes(
          d.label,
        ),
    );
    ...
    if (container.children.length === 0)
      container.innerHTML =
        '<div class="text-slate-500 text-xs col-span-2">No indices data</div>';
  } catch (e) {
    container.innerHTML =
      '<div class="text-slate-500 text-xs col-span-2">Data unavailable</div>';
  }
}
...
async function displayCrypto() {
  ...
  if (!container) return;
  container.innerHTML =
    '<div class="text-slate-500 text-xs py-2">Fetching live prices...</div>';
  try {
    ...
    container.innerHTML =
      '<div class="text-slate-500 text-xs">Data unavailable</div>';
  }
}
async function displayForex() {
  ...
  if (!container) return;
  container.innerHTML =
    '<div class="text-slate-500 text-xs py-2">Fetching live rates...</div>';
  try {
...
async function displayCommodities() {
  ...
  try {
    ...
    if (container.children.length === 0)
      container.innerHTML =
        '<div class="text-slate-500 text-xs col-span-3">No commodity data</div>';
  } catch (e) {
    container.innerHTML =
      '<div class="text-slate-500 text-xs col-span-3">Data unavailable</div>';
  }
...
async function displayCoinGeckoTrending() {
  ...
  if (!container) return;
  container.innerHTML =
    '<div class="text-slate-500 text-xs py-2">Fetching trending...</div>';
  try {
    ...
    container.innerHTML =
      '<div class="text-slate-500 text-xs">Unavailable</div>';
  }
}
...
async function displayCoinGeckoTop10() {
  ...
  if (!container) return;
  container.innerHTML =
    '<div class="text-slate-500 text-xs py-2 col-span-2">Loading top 10...</div>';
  try {
    const cur = (window.currencyCode || "usd").toLowerCase();
    const res = await fetch(
      `https://api.coingecko.com/api/v3/coins/markets?vs_currency=${cur}&order=market_cap_desc&per_page=10&page=1&sparkline=false&price_change_percentage=24h`,
    );
    ...
    container.innerHTML =
      '<div class="text-slate-500 text-xs col-span-2">Unavailable</div>';
  }
}
